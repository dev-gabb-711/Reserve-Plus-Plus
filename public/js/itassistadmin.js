...
function populateFilterOptions() {
  const buildings = getUniqueSortedValues(allTickets.map(ticket => getBuilding(ticket)));
  const rooms = getUniqueSortedValues(allTickets.map(ticket => getRoom(ticket)));
  const categories = getUniqueSortedValues(allTickets.map(ticket => getIssue(ticket)));
...
function getFilteredTickets() {
  return allTickets.filter(ticket => {
    const buildingMatch =
      !selectedFilters.building || getBuilding(ticket) === selectedFilters.building;
    ...
    .map((ticket, index) => {
      return `
        <div class="ticket-card">
          ...
          <div class="ticket-head">
            <div class="t-title">${escapeHtml(getTicketTitle(ticket, index))}</div>
          ...
          <div class="ticket-body">
            <div><span class="k">Lab User:</span> ${escapeHtml(getStudentName(ticket))}</div>
            <div><span class="k">Email:</span> ${escapeHtml(getStudentEmail(ticket))}</div>
            <div><span class="k">Issue:</span> ${escapeHtml(getIssue(ticket))}</div>
            <div><span class="k">Status:</span> ${escapeHtml(ticket.status || "Unresolved")}</div>
            <div><span class="k">Description:</span> ${escapeHtml(getDescription(ticket))}</div>
            <div><span class="k">Submitted:</span> ${escapeHtml(formatDate(ticket.createdAt))}</div>
          </div>

          <div class="ticket-actions">
            <form action="/resolve-ticket/${encodeURIComponent(ticket._id)}" method="POST">
